...
def find():  #To split the dataset into X and Y 
    df = pd.read_csv("F:\CSE ALL COURSES BOOK\Capstone Project Folder\Capstone Code\Capstone Project Code\HDPS-FocusUs-master\HDPS-FocusUs-master\predictor\\heartdata.csv") 
    X = df.iloc[:,:-1].values 
    Y = df.iloc[:,-1:].values
    return X,Y
