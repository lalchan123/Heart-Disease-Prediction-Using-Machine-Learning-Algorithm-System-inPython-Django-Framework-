...
def contact(request): #Diplays contact page . Sends email to HDPS using SMTP.
    if request.method == 'POST':
        ...
        send_mail(title , message+'\n'+'From : '+name+'\n'+'Email : '+email ,from_email=email, recipient_list=['lalchan.badsa.90@gmail.com']) #Sends mail to HDPS
        contactf = ContactModel.objects.create(name=name,email=email,title=title,message=message)
        contactf.save()
...
def signup(request): #For the user to resister or sign up.
    ...
    if request.method == 'POST':
        ...
        if User.objects.filter(username=username).exists():
            ...
            messages.success(request,"Activate your account mail verified.")
            return redirect('signin')
       #return redirect('/')
    else:   
